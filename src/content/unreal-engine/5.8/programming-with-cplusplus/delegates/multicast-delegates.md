---
title: "多播委托"
description: "Multi-cast Delegates in Unreal Engine 的中文译文。"
sourceTitle: "Multi-cast Delegates in Unreal Engine"
sourceUrl: "https://dev.epicgames.com/documentation/unreal-engine/multicast-delegates-in-unreal-engine"
engineVersion: "5.8"
...
多播委托在声明方式上与[声明标准委托](delegates.md)相同，只是前者使用特定于多播委托的宏变体。
...
| `AddLambda()` | 添加一个函子。这通常用于那些不会捕获指针的 lambda 函数。 |
| `AddSPLambda()` | 添加一个函子。它只有在对目标共享指针的弱引用有效的情况下才会被调用。 |
| `AddWeakLambda()` | 添加一个函子。它只有在对 UObject 的弱引用有效的情况下才会被调用。 |
